...
export default function BestSellerProductsScreen() {
  ...
  const fetchBestSellerProducts = async (pageNum = 1, isRefresh = false) => {
    try {
      if (isRefresh) {
        setRefreshing(true);
      } else if (pageNum === 1) {
      ...
      if (response.data && response.data.items) {
        // Filter and sort by totalSoldQuantity
        const bestSellers = response.data.items
        ...
        if (pageNum === 1) {
          ...
          setBestSellerProducts((prev) => [...prev, ...bestSellers]);
        }
  ...
  const onRefresh = () => {
    fetchBestSellerProducts(1, true);
  };
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>
        {t("ecommerce.noProductsFound") || "No best seller products found"}
      </Text>
    </View>
  );
