...
export default function GymsTab({ refreshTrigger }) {
  ...
  const fetchData = async (page = 1, append = false) => {
    try {
      ...
      const gyms = data.gyms?.items || [];
      const total = data.gyms?.totalCount || 0;
      const totalPages = data.gyms?.totalPages || 0;

      if (append) {
        setAllData((prev) => [...prev, ...gyms]);
      } else {
        const featured = gyms.slice(0, Math.min(5, gyms.length));
        const remaining = gyms.slice(Math.min(5, gyms.length));
