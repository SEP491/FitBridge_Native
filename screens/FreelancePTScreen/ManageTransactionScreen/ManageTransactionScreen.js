...
const ManageTransactionScreen = ({ navigation }) => {
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    Promise.all([loadTransactions(), loadWithdrawalHistory()]).finally(() =>
      setRefreshing(false)
    );
  }, []);
  ...
  const handleWithdrawal = async () => {
    ...
    try {
      ...
      const response = await paymentService.sendRequestWithdrawal(
        withdrawalData
      );
      ...
      alert(
        t("withdrawal.requestFailed", "Failed to submit withdrawal request")
      );
    }
  };
