      ...
      <StatRow
        ...
        value={
          stats.firstSessionStartTime
            ? `${new Date(stats.firstSessionStartTime).toLocaleTimeString(
                "en-GB",
                { hour: "2-digit", minute: "2-digit", hour12: false }
              )} - ${new Date(stats.firstSessionStartTime).toLocaleDateString(
                "en-GB"
              )}`
      ...
      <StatRow
        ...
        value={
          stats.latestSessionStartTime
            ? `${new Date(stats.latestSessionStartTime).toLocaleTimeString(
                "en-GB",
                { hour: "2-digit", minute: "2-digit", hour12: false }
              )} - ${new Date(stats.latestSessionStartTime).toLocaleDateString(
                "en-GB"
              )}`
            : t("customerDetail.notAvailable")
        }
